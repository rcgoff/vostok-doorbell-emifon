...
X0003: jb5     TUNEB           ; 0003 - b2 12  2.                      ;A.5=1->>>this is tune from B-set
       mov     a,#A_TBL        ; 0005 - 23 e0  #`                      ;otherwise - A-set
RDSTRT:add     a,r0            ; 0007 - 68     h
...
PL_LOOP:mov    a,r2            ; 000a - fa     z                       ;r2 determines what page to read
       jb2     RD_PG2          ; 000b - 52 0f  R.                      ;...page 2 (if r2.2=1)
       jb5     RD_PG3          ; 000d - b2 16  2.                      ;...page 3 (if r2.5=1)

;-----read page2, if needed
RD_PG2:mov     a,r4            ; 000f - fc     |                       ;r4 determines offset in DATA reading
       jmp     PG2RD           ; 0010 - 44 00  D.

TUNEB: mov     a,#B_TBL        ; 0012 - 23 f0  #p
       jmp     RDSTRT          ; 0014 - 04 07  ..

;-----read page3, if needed
RD_PG3:mov     a,r4            ; 0016 - fc     |                       ;r4 determines offset in DATA reading
       movp3   a,@a            ; 0017 - e3     c                       ;movp3 from @R4
...
PG2RET:jz      ENDPLAY         ; 0018 - c6 75  Fu                      ;zero in data means end of tune
       mov     r1,a            ; 001a - a9     )                       ;backup note byte
       anl     a,#1fh          ; 001b - 53 1f  S.                      ; AND with (0001.1111)=select 5 lower bits
       orl     a,#TONETAB      ; 001d - 43 c0  C@                      ; OR with (1100.0000)=get value like: (110X.XXXX) in range:0c0h-0dfh
       movp    a,@a            ; 001f - a3     #                       ;read byte with this offset (depends on tone)
       mov     r5,a            ; 0020 - ad     -                       ;and save them to R5

       mov     a,r1            ; 0021 - f9     y                       ;process tune byte again
       anl     a,#0e0h         ; 0022 - 53 e0  S`                      ; AND with (1110.0000)=select 3 higher bits from tone byte
       rr      a               ; 0024 - 77     w
       swap    a               ; 0025 - 47     G                       ;move this 3 higher bits to bits 2,1,0.
       ...
       call    DELAY           ; 002a - 34 99  4.                      ;DELAY destroys r1
...
VOLDUR:mov     a,r7            ; 002c - ff     .        1              ;restore note duration address
       ...
       jz      NXNOTE          ; 002e - c6 77  Fw       2(5)           ;00 in dur.table leads to next note
       mov     r1,a            ; 0030 - a9     )        1(6)           ;now r1 has current dur.byte from table

       in      a,p2            ; 0031 - 0a     .        2(8)           ;read previous value on port
       anl     a,#0f8h         ; 0032 - 53 f8  Sx       2(10)          ;AND f8=(1111.1000)=select 5 higher bits
                                                                       ;in other words, clear 3 lowest bits

       xrl     a,r1            ; 0034 - d9     Y        1(11)          ;XOR with duration byte:inverse bit 3 and
                                                                       ;set bits 2..0 to values from duration byte
       orl     a,#0f0h         ; 0035 - 43 f0  Cp       2(13)
       outl    p2,a            ; 0037 - 3a     :        2(15)          ;out lowest nibble

       mov     a,r1            ; 0038 - f9     y        1(16)
       swap    a               ; 0039 - 47     G        1(17)
       anl     a,#0fh          ; 003a - 53 0f  S.       2(19)
...
TMRTON:mov     a,#5dh          ; 003e - 23 5d  #]     2                ;timer is loaded with FIXED value
       mov     t,a             ; 0040 - 62     b      1(3)
       strt    t               ; 0041 - 55     U      1(4)
;--------------tone processing
TONE:  mov     a,r5            ; 0042 - fd     }      1(5)             ;note tone processing
       jz      MUSPAUSE        ; 0043 - c6 56  FV     2(7)             ;tone=0, but duration<>0 -->this is pause
       dec     a               ; 0045 - 07     .                       ;tone (high) loop
       jnz     $-1             ; 0046 - 96 45  .E                      ;cowntdown note value, i.e. the lower the pitch,
                                                                       ;the longer the period and the larger should be
                                                                       ;value in tone array

       in      a,p2            ; 0048 - 0a     .                       ;read value previously sent
       xrl     a,#18h          ; 0049 - d3 18  S.                      ;18h=(0001.1000): invert P2.3 and P2.4
       nop                     ; 004b - 00     .                       ;? - to reach true timing
       outl    p2,a            ; 004c - 3a     :                       ;out nibble again

TMRCHK:jtf     DURAT           ; 004d - 16 5a  .Z     2(13)            ;timer overflow? ->exit from tone player
       mov     a,#23h          ; 004f - 23 23  ##     2(15)            ;time for HIGH P2.3-4 can vary, but time for LOW
       dec     a               ; 0051 - 07     .      1*23h=35(50)     ;P2.3-4 is fixed!
       jnz     $-1             ; 0052 - 96 51  .Q     2*23h=70(120)    ;LOW loop
       jmp     TONE            ; 0054 - 04 42  .B     2(122)
MUSPAUSE:
       anl     p2,#0f7h        ; 0056 - 9a f7  .w     2(9)             ;F7h=(1111.0111) on PAUSE, all is 1 except P2.3
       jmp     TMRCHK          ; 0058 - 04 4d  .M     2(11)            ;in PAUSE,only waiting needed time (set by timer)
...
DURAT: cpl     f1              ; 005a - b5     5       1               ;f1=1 means fixed duration
       jf1     FIXDUR          ; 005b - 76 71  vq      2(3)
       mov     a,r6            ; 005d - fe     ~       1(4)
       jnz     REALDUR         ; 005e - 96 68  .h      2(6)
       ...
       jnz     $-1             ; 0064 - 96 63  .c     b2*25(84)        ;minimal loop
       jmp     VOLDUR          ; 0066 - 04 2c  .,     b2*2(86)         ; and next duration array processing

REALDUR:                                                               ;if realDuration<>0,
       dec     r6              ; 0068 - ce     N      a1(7)            ; dectrease realDuration value,
       mov     a,#1fh          ; 0069 - 23 1f  #.     a2(9)            ; generate some loop-delay,
       nop                     ; 006b - 00     .      a1(10)
REALP: dec     a               ; 006c - 07     .      a1*31(41)
       jnz     REALP           ; 006d - 96 6c  .l     a2*31(103)       ; (means REALdurationLooP)
       jmp     TMRTON          ; 006f - 04 3e  .>     a2(105)          ;and continue current note playing

FIXDUR:mov     a,#20h          ; 0071 - 23 20  #
       jmp     REALP           ; 0073 - 04 6c  .l

ENDPLAY:jmp    PLYRET          ; 0075 - 24 7a  $z                      ;far-jump to exit and continue button polling

NXNOTE:anl     p2,#0f0h        ; 0077 - 9a f0  .p
...
;tune set B
       ...
       db      49h             ; 00f6 - 49     I               ;tune 7b
       db      59h             ; 00f7 - 59     Y               ;tune 8b
       db      69h             ; 00f8 - 69     i               ;tune 9b
       db      7dh             ; 00f9 - 7d     }               ;tune 10b
       db      8ah             ; 00fa - 8a                     ;tune 11b
       db      0a6h            ; 00fb - a6     .&              ;tune 12b
       db      0b3h            ; 00fc - b3     3               ;tune 13b
       db      0c2h            ; 00fd - c2     B               ;tune 14b
       db      0d8h            ; 00fe - d8     X               ;tune 15b
       db      0efh            ; 00ff - ef     o               ;tune 16b
...
INIT:  ANL     P2,#0F0h        ; 0100 - 9A F0                  ;set lower nibble of P2 to zero (on reset they're all '1')
       call    INIT2           ; 0102 - 34 a3  4#              ;set R0=0ffh R1=40h R2=80h (1000.0000) R3=0ffh A=00h and clear RAM at 20h...40h
...
X010b: cpl     f1              ; 010b - b5     5
;---button loop
BUTLOOP:in     a,p1            ; 010c - 09     .               ;read doorbell button (if pressed 1110.1110 (#0EEh), not 1110.1111 (#0EFh)
       orl     a,#0f8h         ; 010d - 43 f8  Cx              ;switch bits 7-3 to 1 (if pressed 1111.1110 (#0FEh), not 1111.1111 (#0FFh)
       cpl     a               ; 010f - 37     7               ;if pressed 0000.0001 (#01h), not 0000.0000 (#00h)
       jz      X0125           ; 0110 - c6 25  F%      ;not pressed - jump to BUTPRES (it may mean: free state, or button release)
       jf0     X0122           ; 0112 - b6 22  6"              ;button was pressed!
       xch     a,r2            ; 0114 - 2a     *               ;save button status to r2
       anl     a,#0f8h         ; 0115 - 53 f8  Sx              ;r2=1v00.0000 AND with 1111.1000 - clear bits 2,1,0
       xrl     a,r2            ; 0117 - da     Z               ;?????r2 XOR
       mov     r2,a            ; 0118 - aa     *
X0119: cpl     f0              ; 0119 - 95     .               ;set 'button pressed' flag
       call    DELAY           ; 011a - 34 99  4.
       call    DELAY           ; 011c - 34 99  4.
       call    DELAY           ; 011e - 34 99  4.
       jmp     BUTLOOP         ; 0120 - 24 0c  $.
;
X0122: clr     f1              ; 0122 - a5     %
       jmp     BUTLOOP         ; 0123 - 24 0c  $.
;BUTPRES---------------------------------------------------------------------------routine if button pressed
X0125: jf0     X0119           ; 0125 - b6 19  6.
       jf1     MLTPRES         ; 0127 - 76 dd  v]              ;MLTPRES - loop-delay for multiple pressings (tune nr increment)
       mov     a,r2            ; 0129 - fa     z               ;select what-to-do by flag register
       jb6     PRESTRT         ; 012a - d2 3f  R?              ;r2.6=button pressed on startup (run demo mode)
       jb2     X0158           ; 012c - 52 58  RX
       jt1     X015c           ; 012e - 56 5c  V\              ;T1= melody fix on/off
       jb1     MEMRIZR         ; 0130 - 32 b4  24              ;r2.1=call MEMORIZER
       in      a,p1            ; 0132 - 09     .               ;read MEMORIZER button (p1.4)
       jb4     X016a           ; 0133 - 92 6a  .j              ;
       mov     a,r2            ; 0135 - fa     z
       cpl     a               ; 0136 - 37     7
       jb4     X013b           ; 0137 - 92 3b  .;              ;r2.4=0 ->skip tune nr.clearing
       mov     r0,#0ffh        ; 0139 - b8 ff  8.              ;tune nr. clearing
X013b: cpl     a               ; 013b - 37     7
       anl     a,#0efh         ; 013c - 53 ef  So              ;r2 and (1110.1111) - clear r2.4 (and exit memorized playing)
       mov     r2,a            ; 013e - aa     *
...
PRESTRT:inc    r0              ; 013f - 18     .               ;tune increment
       mov     a,r2            ; 0140 - fa     z
       anl     a,#0dfh         ; 0141 - 53 df  S_              ;r2 AND 1101.1111 - set bit 5 to 0
       jni     X0147           ; 0143 - 86 47  .G              ;INT= select tune set
       orl     a,#20h          ; 0145 - 43 20  C               ;one case: r2 OR 0010.0000 - set bit 5 to 1
X0147: mov     r2,a            ; 0147 - aa     *               ;write r2

       ;---------------up-to-16 tune count
       mov     a,#10h          ; 0148 - 23 10  #.
X014a: xrl     a,r0            ; 014a - d8     X               ;r0 XOR 0001.0000 and check if zero means test r0.4==1
       jnz     X014f           ; 014b - 96 4f  .O
       mov     r0,#0           ; 014d - b8 00  8.              ;if r0=10h , clr r0. with incr at 1st line of PRESTRT it makes up-to-16 count
       ;---------------------------------

X014f: cpl     f1              ; 014f - b5     5
       mov     r4,#44h         ; 0150 - bc 44  <D              ;starting value for "wait the next button pressing" loop

       mov     a,r2            ; 0152 - fa     z
       anl     a,#7fh          ; 0153 - 53 7f  S.              ;r2 AND 0111.1111 - set bit 7 to 0
       mov     r2,a            ; 0155 - aa     *
       jmp     BUTLOOP         ; 0156 - 24 0c  $.
;-----------------------7-
;
X0158: mov     r4,#0efh        ; 0158 - bc ef  <o              ;0ef in tune set B is Mendelson march, in tune set A this is special beep sequence
       jmp     PL_LOOP         ; 015a - 04 0a  ..
;
X015c: jb1     X01d9           ; 015c - 32 d9  2Y              ;play beep in memo-wr
       jb4     X0166           ; 015e - 92 66  .f              ;or memo-rd mode
       mov     a,r0            ; 0160 - f8     x
       cpl     a               ; 0161 - 37     7
       jz      PRESTRT         ; 0162 - c6 3f  F?
       jmp     X014f           ; 0164 - 24 4f  $O
;
X0166: jb3     X014f           ; 0166 - 72 4f  rO
X0168: jmp     X01d9           ; 0168 - 24 d9  $Y              ;play beep
;
;-----------------------------------------------------------------if p1.4=1 when button released
;------------------------------------------------------seems to me this is entrance to memorizing mode
X016a: mov     a,r2            ; 016a - fa     z
       jb4     X016f           ; 016b - 92 6f  .o              ;memorizer already on? yes - skip reset of tune counter
       mov     r0,#0ffh        ; 016d - b8 ff  8.              ;r2.4=1 -> reset tune counter
X016f: orl     a,#10h          ; 016f - 43 10  C.              ;r2 OR 0001.0000==r2.4=1 (switch memorized playing on)
       mov     r2,a            ; 0171 - aa     *
       cpl     a               ; 0172 - 37     7
       jb3     X01d9           ; 0173 - 72 d9  rY              ;play beep
       inc     r0              ; 0175 - 18     .               ;next tune
       mov     a,r3            ; 0176 - fb     {
       inc     a               ; 0177 - 17     .
       jmp     X014a           ; 0178 - 24 4a  $J

;---------------------------------------------------------------------------------------RET_FROM_PLAY
PLYRET:mov     a,r2            ; 017a - fa     z               ;returning point from sound playing
       jb6     DEMRET          ; 017b - d2 91  R.              ;one case is demo mode
       jt0     X0183           ; 017d - 36 83  6.              ;other - Step-up count...
       jt1     X0183           ; 017f - 56 83  V.              ;...or Tune fix mode
X0181: mov     r0,#0ffh        ; 0181 - b8 ff  8.              ;else - reset tune number
X0183: anl     p2,#0f0h        ; 0183 - 9a f0  .p              ;P2 and 1111.0000
X0185: orl     p2,#10h         ; 0185 - 8a 10  ..              ;P2 or 0001.0000 - i.e. set p2 to: xxx1.0000
       mov     a,r2            ; 0187 - fa     z
       anl     a,#0b8h         ; 0188 - 53 b8  S8              ;r2 AND 1011.1000 clears bit 6(demo) and bits 2, 1 (memorizing), 0
       orl     a,#80h          ; 018a - 43 80  C.              ;r2 OR 1000.0000 sets bit 7
       mov     r2,a            ; 018c - aa     *
       clr     f1              ; 018d - a5     %
       cpl     f1              ; 018e - b5     5               ;set f1
...
DEMRET:inc     r0              ; 0191 - 18     .               ;next tune
       mov     a,r0            ; 0192 - f8     x
       xrl     a,#10h          ; 0193 - d3 10  S.
       jz      X0181           ; 0195 - c6 81  F.              ;tune nr. exceeds 0f? this will end demo mode
       jmp     DEMCONT         ; 0197 - 24 e6  $f              ;otherwise - demo continue

;DELAY-----------------------------------------------------------------------------------------delay routine on A and R1
DELAY: mov     r1,#6           ; 0199 - b9 06  9.
OUTER: mov     a,#0dfh         ; 019b - 23 df  #_
INNER: dec     a               ; 019d - 07     .               ;1 cycle
       jnz     INNER           ; 019e - 96 9d  ..              ;2 cycles
       djnz    r1,OUTER        ; 01a0 - e9 9b  i.
       retr                    ; 01a2 - 93     .


;-------------------------------------------------------------------------------------------INIT2
;-------------------------------------------------------------------------------------------clearing RAM and some other registers
INIT2: mov     r0,#0ffh        ; 01a3 - b8 ff  8.
CLRAM: mov     r3,#0ffh        ; 01a5 - bb ff  ;.
       mov     r1,#20h         ; 01a7 - b9 20  9
X01a9: mov     @r1,#0          ; 01a9 - b1 00  1.              ;some data structure in RAM (starting from #20h...
       inc     r1              ; 01ab - 19     .
...
MEMRIZR:       in      a,p1            ; 01b4 - 09     .
       jb4     X01d7           ; 01b5 - 92 d7  .W              ;check if p1.4=1, if yes, clear RAM and play BEEP2
       mov     a,r3            ; 01b7 - fb     {
       xrl     a,#0fh          ; 01b8 - d3 0f  S.
       jz      X01d9           ; 01ba - c6 d9  FY              ;r3=0fh (memory overflow) ->  play BEEP2
       mov     a,r0            ; 01bc - f8     x
       cpl     a               ; 01bd - 37     7
       jz      X01d9           ; 01be - c6 d9  FY              ;r0=ffh (first button pressing) -> play BEEP2
       mov     a,r2            ; 01c0 - fa     z
       orl     a,#8            ; 01c1 - 43 08  C.              ;r2 OR 0000.1000 = set bit 3
       mov     r2,a            ; 01c3 - aa     *
       inc     r3              ; 01c4 - 1b     .               ;increment address for memorizing
       mov     a,r3            ; 01c5 - fb     {
       orl     a,#20h          ; 01c6 - 43 20  C               ;r1:=r3+20h=address for memorizing
       mov     r1,a            ; 01c8 - a9     )
       mov     a,#0e0h         ; 01c9 - 23 e0  #`              ;select tune set depending on INT button: choose offset in table
       jni     X01cf           ; 01cb - 86 cf  .O
       mov     a,#0f0h         ; 01cd - 23 f0  #p
X01cf: orl     a,r0            ; 01cf - 48     H               ;r0(tune nr.)+offset depending on tune set=addr of starting byte value in 0e0-0ff table
       mov     @r1,a           ; 01d0 - a1     !               ;write into RAM tune offset
       mov     r0,#0ffh        ; 01d1 - b8 ff  8.              ;reset tune nr
       mov     a,#7eh          ; 01d3 - 23 7e  #~              ;a:=0111.1110
       jmp     X01f2           ; 01d5 - 24 f2  $r              ;start playing with start address @07eh (play BEEP1)
;
X01d7: call    CLRAM           ; 01d7 - 34 a5  4%              ;part of INIT2 - clearing RAM
X01d9: mov     a,#7fh          ; 01d9 - 23 7f  #.
       jmp     X01f2           ; 01db - 24 f2  $r              ;start playing with start adress @07fh (play BEEP2)
;
...
MLTPRES:mov    a,r2            ; 01dd - fa     z               ;here we at startup if button was not pressed or was released
       jb7     BUTLOOP         ; 01de - f2 0c  r.
       dec     r4              ; 01e0 - cc     L
       call    DELAY           ; 01e1 - 34 99  4.
       mov     a,r4            ; 01e3 - fc     |
...
DEMCONT:mov    a,r2            ; 01e6 - fa     z
       jb4     MEMOPLAY        ; 01e7 - 92 eb  .k              ;playing tune in memorized seuence if r2.4=1
...
MEMOPLAY:
       mov     a,r0            ; 01eb - f8     x
       orl     a,#20h          ; 01ec - 43 20  C               ;??offset 20h+tune number
       mov     r1,a            ; 01ee - a9     )
       mov     a,@r1           ; 01ef - f1     q               ;??read RAM tune byte from 20h+tune nr.
       jb4     X01f8           ; 01f0 - 92 f8  .x              ;set r2.5 value (tune set) depending on bit 4 of RAM-read value
                                                               ;start offset 0E0h:tune setA:bit4=0; offset 0D0h:set B:bit4=1
X01f2: xch     a,r2            ; 01f2 - 2a     *
       anl     a,#0dfh         ; 01f3 - 53 df  S_              ;r2 AND (1101.1111)=r2.5:=0
...
X01f8: xch     a,r2            ; 01f8 - 2a     *
       orl     a,#20h          ; 01f9 - 43 20  C               ;r2 OR (0010.1111)=r2.5:=1 (set B)
       jmp     X01f5           ; 01fb - 24 f5  $u
